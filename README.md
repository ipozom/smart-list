...
SmartList is a minimal, local-first Android sample demonstrating Jetpack Compose + Room + ViewModel patterns.

This repository's active app module is `smartlist/`. The previous `app/` module was moved to `archive/app-backup/` during the refactor.

Quick summary

- Two primary screens: Lists (shows list names) and Items (items for a selected list).
...
2. Build and run the `:smartlist` module on an emulator or device.
...
See `docs/UPDATES.md` for a detailed changelog, design notes, testing guidance, and recommended next steps.

Notes & caveats

- The app uses an in-memory undo buffer; undo data is lost if the process is killed.
- During development Room may use `fallbackToDestructiveMigration()`; replace with explicit `Migration` objects before releasing to users.

Testing

- Unit tests: add JVM tests for ViewModels (fake DAOs) to validate add/rename/delete and undo logic.
- Instrumentation: run connected tests for end-to-end flows against an emulator or device.

If you want, I can add a small set of ViewModel unit tests next or create an instrumentation test for delete+undo.

---

If anything here looks incorrect for your setup, tell me what to change and I will update the README accordingly.
