...
class Repository(
    ...
    suspend fun updateList(list: ListEntity) {
        ...
        Log.d("Repository", "updateList: id=${updated.id} title=${updated.title}")
        // Prefer update if available
        try {
            listDao.update(updated)
        } catch (e: NoSuchMethodError) {
            // Fallback to insert/replace just in case the DAO doesn't support update (legacy)
            listDao.insert(updated)
        }
    }
